...
4. Run Backtesting:
	a. Ensure that backtesting data is available in "data" folder
	b. Run executable after compilation is complete 
	c. Trade log results will available in "logs" folder:
		# Log filename format is: tradeLog_XXXXXX_MMMYYYY_type_timestamp.txt (e.g. tradeLog_EURUSD_Apr2024_1s_1719630601.txt)
	d. Each chunk in the log file contains:
	...
	e. P&L results will available in "results" folder:
	...
	f. The P&L results filename and Log filename have the same "timestamp" extension, so that it's easy to pair them up.