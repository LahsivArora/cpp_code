    ...
    while (!xData.empty()){
        ...
        if ((newPrice <= currentPrice + tolerance) && no_trend_live){
            ...
            if (!no_trend_live){
                ...
                if (newPrice < currentPrice){ xTrades->back().stopLoss += 0.003; currentTrade.stopLoss += 0.003;}
                if (newPrice > currentPrice){ xTrades->back().takeProfit += 0.003; currentTrade.takeProfit += 0.003;}
                if (newPrice > currentPrice){ xTrades->back().killTime += 0.75; currentTrade.killTime += 0.75;}
